...
(define (dot-product X Y)
  (if (null? X) 0 (+ (* (car X) (car Y)) (dot-product (cdr X) (cdr Y))))
 )
...
(define (multiply-queue M V L acc i)
  (cond
    ((null? M) (reverse (cons acc L)))
    ((zero? i) (multiply-queue (cdr M) V L (dot-product (car M) V) (+ i 1)))
    (else (multiply-queue (cdr M) V (cons acc L) (dot-product (car M) V) (+ i 1)))
  )
 )
(define (multiply M V)
  
  (multiply-queue M V '() 0 0)
 )
...
(define (get-one-transformation n)
  (modulo n 3)
 )
...
(define (get-transformations-helper n result)
  (cond
    ((= n 1) result)
    ((= (get-one-transformation n) 1) (get-transformations-helper (quotient (+ n 1) 3) (cons 3 result)))
    ((= (get-one-transformation n) 2) (get-transformations-helper (quotient (+ n 1) 3) (cons 1 result)))
    (else (get-transformations-helper (quotient (+ n 1) 3) (cons 2 result)))
   )
  )
...
; Utilizați recursivitate pe coadă.
(define (apply-matrix-transformations-queue Ts ppt)
  (cond
    ((null? Ts) ppt)
    ((= (car Ts) 1) (apply-matrix-transformations-queue (cdr Ts) (multiply T1 ppt)))
    ((= (car Ts) 2) (apply-matrix-transformations-queue (cdr Ts) (multiply T2 ppt)))
    ((= (car Ts) 3) (apply-matrix-transformations-queue (cdr Ts) (multiply T3 ppt)))
    )
 )

(define (apply-matrix-transformations Ts ppt)
  (apply-matrix-transformations-queue Ts ppt)
 )
