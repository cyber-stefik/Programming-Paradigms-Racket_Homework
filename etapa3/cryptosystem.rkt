...
(define cryptList (list a1 b1 c1 a2 b2 c2 a3 b3 c3))
(define (key n)
  (define myQuadruple (get-nth-quadruple n))
  (define res '())
  (let ([e (list-ref myQuadruple 1)] [f (list-ref myQuadruple 2)])
    (foldr (λ (Fs res) (cons (modulo (Fs e f) 27) res)) '() cryptList)
   )
 )
...
(define (message->codes message)
  (define stringList (string->list message))
  (let myFunc ([stringList stringList] [res '()])
    (cond
      ((null? stringList) (reverse res))
      ((= (char->integer (car stringList)) 32) (myFunc (cdr stringList) (cons 0 res))) 
      (else (myFunc (cdr stringList) (cons (- (char->integer (car stringList)) 96) res)))
     )
   )
 )
...
(define (codes->message codes)
  (let myFunc ([codes codes] [res '()])
    (cond
      ((null? codes) (list->string (reverse res)))
      ((zero? (car codes)) (myFunc (cdr codes) (cons (integer->char 32) res))) 
      (else (myFunc (cdr codes) (cons (integer->char (+ (car codes) 96)) res)))
     )
   )
 )
...
(define (createList key size res)
  (if (zero? size)
      res
      (createList key (- size 1) (append key res))
   )
 )

(define (extend-key key size)
  (define newKey (createList key (+ (quotient size (length key)) 1) '()))
  (let myFunc ([newKey newKey] [size size] [res '()])
             (cond
               ((or (zero? size) (null? newKey)) (reverse res))
               (else (myFunc (cdr newKey) (- size 1) (cons (car newKey) res)))
              )
   )
 )
...
(define (myCodes f)
  (λ (message key)
    (let ([newKey (extend-key key (length message))])
      (map (λ (x y) (modulo (f x y) 27)) message newKey))
   )
 )
...
(define (myMessages f)
  (λ (message key)
    (codes->message (f (message->codes message) key))
   )
 )
(define encrypt-message
  (myMessages encrypt-codes))
(define decrypt-message
  (myMessages decrypt-codes))
