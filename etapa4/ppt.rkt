...
(define ppt-stream-in-tree-order
    (let iter ([myQ (stream-cons (list 3 4 5) empty-stream)])
      (stream-cons (stream-first myQ) (iter (stream-append (stream-rest myQ)
                                                           (list (multiply T1 (stream-first myQ)))
                                                           (list (multiply T2 (stream-first myQ)))
                                                           (list (multiply T3 (stream-first myQ)))))
     ))
 )
...
(define (pairs G H)
  (let iter ([H H] [i 0])
    (let iter2 ([j 0] [G G])
      (cond
        ((> j i) (iter (stream-rest H) (+ i 1)))
        (else (stream-cons (cons (stream-first G) (stream-first H)) (iter2 (+ j 1) (stream-rest G))))
       )
     )
   )
 )
...
(define gh-pairs-stream
  (stream-filter
   (λ (x) (and (= 1 (gcd (car x) (cdr x))) (and (odd? (car x)) (odd? (cdr x)))))
   (pairs naturals (stream-rest naturals))
   )
  )
...
(define ppt-stream-in-pair-order
  (let iter ([F gh-pairs-stream])
    (stream-cons
     (list (* (car (stream-first F)) (cdr (stream-first F)))
           (quotient (- (expt (cdr (stream-first F)) 2) (expt (car (stream-first F)) 2)) 2)
           (quotient (+ (expt (car (stream-first F)) 2) (expt (cdr (stream-first F)) 2)) 2)) (iter (stream-rest F)))
   )
 )
